...
namespace libcLdvben50221 {
   ...
   typedef struct en50221_session_t {
         int i_slot;
         int i_resource_id;
         void (* pf_handle)(int, uint8_t *, int );
         void (* pf_close)(int );
         void (* pf_manage)(int );
         void *p_sys;
   } en50221_session_t;
    ...
    * Local prototypes
    *****************************************************************************/
   static void SessionOpenCb( uint8_t i_slot );
   static void SPDUHandle( uint8_t i_slot,
         uint8_t *p_spdu, int i_size );

   static void ResourceManagerOpen( int i_session_id );
   static void ApplicationInformationOpen( int i_session_id );
   static void ConditionalAccessOpen( int i_session_id );
   static void DateTimeOpen( int i_session_id );
   static void ResetSlotCb(void *loop, void *w, int revents);
   static void en50221_Read(void *loop, void *w, int revents);
   static void en50221_Poll(void *loop, void *w, int revents);
   static void MMIOpen( int i_session_id );
...
#define MAX_DUMP 256
    ...
    * TPDUWrite
    *****************************************************************************/
   static int TPDUWrite( uint8_t i_slot )
   {
    ...
    * TPDUSend
    *****************************************************************************/
   static int TPDUSend( uint8_t i_slot, uint8_t i_tag,
   ...
   static int TPDURecv( )
   {
      ...
      switch ( i_tag )
      {
         case T_CTC_REPLY:
            ...
            cLev_timer_stop(libcLdvb::event_loop, &p_slot->init_watcher);
         ...
         case T_DATA_MORE:
         {
            ...
            if ( b_last )
            {
               SPDUHandle(i_slot, p_recv->p_data, p_recv->i_size );
               free( p_recv->p_data );
               free( p_recv );
               p_slot->p_recv = NULL;
            }
      ...
      if ( !p_slot->b_expect_answer && p_slot->i_pending_session_id != 0 )
         SessionOpenCb(i_slot );
      if ( !p_slot->b_expect_answer && p_slot->b_has_data )
         TPDUSend(i_slot, T_RCV, NULL, 0 );
...
#define RI_MMI                         0x00400041
    ...
    * SPDUSend
    *****************************************************************************/
   static int SPDUSend( int i_session_id,
         uint8_t *p_data, int i_size )
   {
      ...
      *p++ = i_session_id & 0xff;
      ...
      while ( i_size > 0 )
      {
         ...
         {
            if ( TPDUSend(i_slot, T_DATA_MORE, p,
                  MAX_TPDU_DATA ) != 0 )
            {
         ...
         {
            if ( TPDUSend(i_slot, T_DATA_LAST, p, i_size )
                  != 0 )
            {
               cLbugf(cL::dbg_dvb, "couldn't send TPDU on session %d\n", i_session_id );
               free( p_spdu );
   ...
   static void SessionOpenCb( uint8_t i_slot )
   {
      ...
      switch ( i_resource_id )
      {
         case RI_RESOURCE_MANAGER:
            ResourceManagerOpen(i_session_id ); break;
         case RI_APPLICATION_INFORMATION:
            ApplicationInformationOpen(i_session_id ); break;
         case RI_CONDITIONAL_ACCESS_SUPPORT:
            ConditionalAccessOpen(i_session_id ); break;
         case RI_DATE_TIME:
            DateTimeOpen(i_session_id ); break;
         case RI_MMI:
            MMIOpen(i_session_id ); break;
   ...
   static void SessionOpen( uint8_t i_slot,
         uint8_t *p_spdu, int i_size )
   {
      ci_slot_t *p_slot = &p_slots[i_slot];
      int i_session_id;
      ...
      if ( TPDUSend(i_slot, T_DATA_LAST, p_response, 9 ) != 0 )
      {
         cLbugf(cL::dbg_dvb,
               "SessionOpen: couldn't send TPDU on slot %d\n", i_slot );
         return;
      }
    ...
    * SessionCreate
    *****************************************************************************/
   static void SessionCreate( int i_slot, int i_resource_id )
   {
      uint8_t p_response[16];
      uint8_t i_tag;
      int i_session_id;
      ...
      if ( TPDUSend(i_slot, T_DATA_LAST, p_response, 4 ) !=
            0 )
      {
         cLbugf(cL::dbg_dvb,
               "SessionCreate: couldn't send TPDU on slot %d\n", i_slot );
         return;
      }
   }
#endif
    ...
    *****************************************************************************/
   static void SessionCreateResponse( uint8_t i_slot,
         uint8_t *p_spdu, int i_size )
   {
      ...
      if ( i_status != SS_OK )
      {
         cLbugf(cL::dbg_dvb, "SessionCreateResponse: failed to open session %d"
    ...
    * SessionSendClose
    *****************************************************************************/
   static void SessionSendClose( int i_session_id )
   {
      uint8_t p_response[16];
      uint8_t i_slot = p_sessions[i_session_id - 1].i_slot;
    ...
    * SessionClose
    *****************************************************************************/
   static void SessionClose( int i_session_id )
   {
      uint8_t p_response[16];
      uint8_t i_slot = p_sessions[i_session_id - 1].i_slot;

      if ( p_sessions[i_session_id - 1].pf_close != NULL )
         p_sessions[i_session_id - 1].pf_close(i_session_id );
      p_sessions[i_session_id - 1].i_resource_id = 0;
      ...
      if ( TPDUSend(i_slot, T_DATA_LAST, p_response, 5 ) !=
            0 )
      {
      ...
      switch ( p_spdu[0] )
      {
         case ST_SESSION_NUMBER:
            ...
            if ( i_session_id <= MAX_SESSIONS
               ...
               p_sessions[i_session_id - 1].pf_handle(i_session_id,
                     p_spdu + 4, i_size - 4 );
            break;
         ...
         case ST_OPEN_SESSION_REQUEST:
            if ( i_size != 6 || p_spdu[1] != 0x4 )
               return;
            SessionOpen(i_slot, p_spdu, i_size );
            break;

         case ST_CREATE_SESSION_RESPONSE:
            if ( i_size != 9 || p_spdu[1] != 0x7 )
               return;
            SessionCreateResponse(i_slot, p_spdu, i_size );
            break;
...
#define AOT_COMMS_RCV_MORE          0x9F8C06
    ...
    * APDUSend
    *****************************************************************************/
   static int APDUSend( int i_session_id, int i_tag,
         uint8_t *p_data, int i_size )
   {
      ...
      *p++ = i_tag & 0xff;
      ...
      {
         i_ret = SPDUSend(i_session_id, p_apdu, i_size + p - p_apdu );
      }
      else
      {
    ...
    *****************************************************************************/
   static void ResourceManagerHandle( int i_session_id,
         uint8_t *p_apdu, int i_size )
   {
      ...
      switch ( i_tag )
      {
         case AOT_PROFILE_ENQ:
         {
            ...
            APDUSend(i_session_id, AOT_PROFILE, (uint8_t*)resources,
                  sizeof(resources) );
            break;
         }
         case AOT_PROFILE:
            APDUSend(i_session_id, AOT_PROFILE_CHANGE, NULL, 0 );
            break;
   ...
   static void ResourceManagerOpen( int i_session_id )
   {
      ...
      APDUSend(i_session_id, AOT_PROFILE_ENQ, NULL, 0 );
   }
    ...
    *****************************************************************************/
   static void ApplicationInformationEnterMenu(int i_session_id )
   {
      int i_slot = p_sessions[i_session_id - 1].i_slot;

      cLbugf(cL::dbg_dvb, "entering MMI menus on session %d\n", i_session_id );
      APDUSend(i_session_id, AOT_ENTER_MENU, NULL, 0 );
    ...
    *****************************************************************************/
   static void ApplicationInformationHandle( int i_session_id,
         uint8_t *p_apdu, int i_size )
   {
      int i_tag = APDUGetTag( p_apdu, i_size );
   ...
   static void ApplicationInformationOpen( int i_session_id )
   {
      ...
      APDUSend(i_session_id, AOT_APPLICATION_INFO_ENQ, NULL, 0 );
   }
   ...
   static uint8_t *CAPMTBuild( int i_session_id, uint8_t *p_pmt, uint8_t i_list_mgt, uint8_t i_cmd, int *pi_capmt_size )
   {
      system_ids_t *p_ids = (system_ids_t *)p_sessions[i_session_id - 1].p_sys;
      uint8_t *p_es;
      ...
      CopyCADescriptors( p_ids, i_cmd, capmt_get_infos( p_capmt ), pmt_get_descs( p_pmt ) );
      ...
      while ( (p_es = pmt_get_es( p_pmt, j )) != NULL ) {
         ...
         CopyCADescriptors( p_ids, i_cmd, capmtn_get_infos( p_capmt_n ), pmtn_get_descs( p_es ) );
      }
    ...
    * CAPMTFirst
    *****************************************************************************/
   static void CAPMTFirst( int i_session_id, uint8_t *p_pmt )
   {
      uint8_t *p_capmt;
      int i_capmt_size;
      ...
      p_capmt = CAPMTBuild(i_session_id, p_pmt,
            0x3 /* only */, 0x1 /* ok_descrambling */,
            &i_capmt_size );

      if ( i_capmt_size )
      {
         APDUSend(i_session_id, AOT_CA_PMT, p_capmt, i_capmt_size );
         free( p_capmt );
      }
   }
    ...
    * CAPMTAdd
    *****************************************************************************/
   static void CAPMTAdd( int i_session_id, uint8_t *p_pmt )
   {
      system_ids_t *p_ids =
    ...
    * CAPMTUpdate
    *****************************************************************************/
   static void CAPMTUpdate( int i_session_id, uint8_t *p_pmt )
   {
      uint8_t *p_capmt;
      int i_capmt_size;
    ...
    * CAPMTDelete
    *****************************************************************************/
   static void CAPMTDelete( int i_session_id, uint8_t *p_pmt )
   {
      system_ids_t *p_ids =
            (system_ids_t *)p_sessions[i_session_id - 1].p_sys;
      uint8_t *p_capmt;
    ...
    *****************************************************************************/
   static void ConditionalAccessHandle( int i_session_id,
         uint8_t *p_apdu, int i_size )
   {
      system_ids_t *p_ids =
            (system_ids_t *)p_sessions[i_session_id - 1].p_sys;
    ...
    *****************************************************************************/
   static void ConditionalAccessClose( int i_session_id )
   {
   ...
   static void ConditionalAccessOpen( int i_session_id )
   {
      ...
      p_sessions[i_session_id - 1].p_sys = (void *)cLmalloc(system_ids_t, 1);
      memset( p_sessions[i_session_id - 1].p_sys, 0,
            sizeof(system_ids_t) );

      APDUSend(i_session_id, AOT_CA_INFO_ENQ, NULL, 0 );
   }
    ...
    * DateTimeSend
    *****************************************************************************/
   static void DateTimeSend(int i_session_id )
   {
      date_time_t *p_date =
...
#define DEC2BCD(d) (((d / 10) << 4) + (d % 10))
         ...
         APDUSend(i_session_id, AOT_DATE_TIME, p_response, 7 );

         cLev_timer_again(libcLdvb::event_loop, &p_date->watcher);
      }
   }
   ...
   static void _DateTimeSend(void *loop, void *p, int revents)
   {
      ...
      DateTimeSend(p_date->i_session_id );
   }

   /*****************************************************************************
    * DateTimeHandle
    *****************************************************************************/
   static void DateTimeHandle( int i_session_id,
         uint8_t *p_apdu, int i_size )
   {
      date_time_t *p_date =
            (date_time_t *)p_sessions[i_session_id - 1].p_sys;
      ...
      switch ( i_tag )
      {
         case AOT_DATE_TIME_ENQ:
         {
            ...
            cLev_timer_stop(libcLdvb::event_loop, &p_date->watcher);
            cLev_timer_set(&p_date->watcher, p_date->i_interval, p_date->i_interval);
            DateTimeSend(i_session_id );
            break;
         }
         default:
    ...
    * DateTimeClose
    *****************************************************************************/
   static void DateTimeClose( int i_session_id )
   {
      date_time_t *p_date =
            (date_time_t *)p_sessions[i_session_id - 1].p_sys;
      cLev_timer_stop(libcLdvb::event_loop, &p_date->watcher);
   ...
   static void DateTimeOpen( int i_session_id )
   {
      ...
      p_sessions[i_session_id - 1].p_sys = (void *)cLmalloc(date_time_t, 1);
      memset( p_sessions[i_session_id - 1].p_sys, 0, sizeof(date_time_t) );

      date_time_t *p_date =
            (date_time_t *)p_sessions[i_session_id - 1].p_sys;
      p_date->i_session_id = i_session_id;
      cLev_timer_init(&p_date->watcher, _DateTimeSend, 0, 0);
...
#define AI_ANSWER  0x01
    ...
    * MMISendObject
    *****************************************************************************/
   static void MMISendObject( int i_session_id,
         en50221_mmi_object_t *p_object )
   {
      ...
      APDUSend(i_session_id, i_tag, p_data, i_size );
      free( p_data );
    ...
    * MMISendClose
    *****************************************************************************/
   static void MMISendClose( int i_session_id )
   {
      int i_slot = p_sessions[i_session_id - 1].i_slot;

      APDUSend(i_session_id, AOT_CLOSE_MMI, NULL, 0 );
    ...
    * MMIDisplayReply
    *****************************************************************************/
   static void MMIDisplayReply( int i_session_id )
   {
      ...
      APDUSend(i_session_id, AOT_DISPLAY_REPLY, p_response, 2 );
    ...
    * MMIHandleEnq
    *****************************************************************************/
   static void MMIHandleEnq( int i_session_id,
         uint8_t *p_apdu, int i_size )
   {
    ...
    * MMIHandleMenu
    *****************************************************************************/
   static void MMIHandleMenu( int i_session_id, int i_tag,
         uint8_t *p_apdu, int i_size )
   {
      mmi_t *p_mmi = (mmi_t *)p_sessions[i_session_id - 1].p_sys;
      int i_slot = p_sessions[i_session_id - 1].i_slot;
...
#undef GET_FIELD
    ...
    * MMIHandle
    *****************************************************************************/
   static void MMIHandle( int i_session_id,
         uint8_t *p_apdu, int i_size )
   {
      int i_tag = APDUGetTag( p_apdu, i_size );
      ...
      switch ( i_tag )
      {
         ...
         case AOT_ENQ:
            MMIHandleEnq(i_session_id, p_apdu, i_size );
            break;

         case AOT_LIST_LAST:
         case AOT_MENU_LAST:
            MMIHandleMenu(i_session_id, i_tag, p_apdu, i_size );
            break;

         case AOT_CLOSE_MMI:
            SessionSendClose(i_session_id );
            break;
    ...
    * MMIClose
    *****************************************************************************/
   static void MMIClose( int i_session_id )
   {
      int i_slot = p_sessions[i_session_id - 1].i_slot;
      mmi_t *p_mmi = (mmi_t *)p_sessions[i_session_id - 1].p_sys;
   ...
   static void MMIOpen( int i_session_id )
   {
      ...
      p_sessions[i_session_id - 1].p_sys = (void *)cLmalloc(mmi_t, 1);
    ...
    *****************************************************************************/
   static void InitSlot( int i_slot )
   {
      if ( TPDUSend(i_slot, T_CREATE_TC, NULL, 0 ) != 0 )
   ...
   static void ResetSlot( int i_slot )
   {
      ...
      cLev_timer_init(&p_slot->init_watcher, ResetSlotCb, CAM_INIT_TIMEOUT / 1000000., 0);
      cLev_timer_start(libcLdvb::event_loop, &p_slot->init_watcher);
      p_slot->b_expect_answer = false;
   ...
   static void ResetSlotCb(void *loop, void *p, int revents)
   {
      struct cLev_timer *w = (struct cLev_timer *)p;
   ...
   void en50221_Init( void )
   {
      ...
      if( i_ca_type & CA_CI_LINK )
      {
         cLev_io_init(&cam_watcher, en50221_Read, i_ca_handle, 1); //EV_READ
         cLev_io_start(libcLdvb::event_loop, &cam_watcher);

         cLev_timer_init(&slot_watcher, en50221_Poll, CA_POLL_PERIOD / 1000000.,
               CA_POLL_PERIOD / 1000000.);
         cLev_timer_start(libcLdvb::event_loop, &slot_watcher);
      }
   ...
   void en50221_Reset( void )
   {
      ...
      else
      {
         ...
         if ( p_sessions[0].p_sys == NULL )
            p_sessions[0].p_sys = (void *)cLmalloc(system_ids_t, 1);
         ...
         APDUSend(1, AOT_APPLICATION_INFO_ENQ, NULL, 0 );
   ...
   static void en50221_Read(void *loop, void *w, int revents)
   {
      ...
      cLev_timer_again(libcLdvb::event_loop, &slot_watcher);
   }
   ...
   static void en50221_Poll(void *loop, void *w, int revents)
   {
      ...
      for ( i_session_id = 1; i_session_id <= MAX_SESSIONS; i_session_id++ )
      {
         ...
         if ( p_session->i_resource_id && p_session->pf_manage != NULL
            ...
            p_session->pf_manage(i_session_id );
      }
      ...
      for ( i_slot = 0; i_slot < i_nb_slots; i_slot++ )
      {
         ...
         if ( p_slot->b_active && !p_slot->b_expect_answer )
         {
            if ( TPDUSend(i_slot, T_DATA_LAST, NULL, 0 ) != 0 )
            {
   ...
   void en50221_UpdatePMT( uint8_t *p_pmt )
   {
      ...
      for ( i_session_id = 1; i_session_id <= MAX_SESSIONS; i_session_id++ )
         if ( p_sessions[i_session_id - 1].i_resource_id
               == RI_CONDITIONAL_ACCESS_SUPPORT )
            CAPMTUpdate(i_session_id, p_pmt );
   }
   ...
   void en50221_DeletePMT( uint8_t *p_pmt )
   {
      ...
      for ( i_session_id = 1; i_session_id <= MAX_SESSIONS; i_session_id++ )
         if ( p_sessions[i_session_id - 1].i_resource_id
               == RI_CONDITIONAL_ACCESS_SUPPORT )
            CAPMTDelete(i_session_id, p_pmt );
   }
   ...
   uint8_t en50221_StatusMMI( uint8_t *p_answer, ssize_t *pi_size )
   {
      ...
      *pi_size = sizeof(struct ret_mmi_status);
      return libcLdvben50221::RET_MMI_STATUS;
   }
         ...
         uint8_t *p_answer, ssize_t *pi_size )
   {
      ...
      if ( i_size != 1 ) return libcLdvben50221::RET_HUH;
      i_slot = *p_buffer;
      ...
      *pi_size = sizeof(struct ret_mmi_slot_status);
      return libcLdvben50221::RET_MMI_SLOT_STATUS;
   }
      ...
      if( i_ca_type & CA_CI_LINK )
      {
         ...
         for ( i_session_id = 1; i_session_id <= MAX_SESSIONS; i_session_id++ )
         {
            if ( p_sessions[i_session_id - 1].i_resource_id
                  ...
                  && p_sessions[i_session_id - 1].i_slot == i_slot )
            {
               ApplicationInformationEnterMenu(i_session_id );
               return libcLdvben50221::RET_OK;
            }
         }
   ...
   uint8_t en50221_GetMMIObject( uint8_t *p_buffer, ssize_t i_size, uint8_t *p_answer, ssize_t *pi_size )
   {
      ...
      if ( p_slots[i_slot].b_mmi_expected )
         return libcLdvben50221::RET_MMI_WAIT; /* data not yet available */
      ...
      *pi_size = sizeof(struct ret_mmi_recv);
      ...
      for ( i_session_id = 1; i_session_id <= MAX_SESSIONS; i_session_id++ )
      {
               ...
               && p_sessions[i_session_id - 1].i_slot == i_slot )
         {
            ...
            {
               *pi_size = 0;
               return libcLdvben50221::RET_ERR; /* should not happen */
            }

            *pi_size = CLDVB_COMM_BUFFER_SIZE - CLDVB_COMM_HEADER_SIZE -  (&p_ret->object - (libcLdvben50221::en50221_mmi_object_t *)p_ret);
            ...
            *pi_size += (&p_ret->object - (libcLdvben50221::en50221_mmi_object_t *)p_ret);
            break;
         }
      }

      return libcLdvben50221::RET_MMI_RECV;
   }
   ...
   uint8_t en50221_SendMMIObject( uint8_t *p_buffer, ssize_t i_size )
   {
      ...
      if ( (unsigned long)i_size < sizeof(struct cmd_mmi_send))
      {
         ...
         return libcLdvben50221::RET_HUH;
      }

      if ( en50221_UnserializeMMIObject( &p_cmd->object, i_size - (&p_cmd->object - (libcLdvben50221::en50221_mmi_object_t *)p_cmd) ) == -1 )
         return libcLdvben50221::RET_ERR;
      ...
      for ( i_session_id = 1; i_session_id <= MAX_SESSIONS; i_session_id++ )
      {
         if ( p_sessions[i_session_id - 1].i_resource_id == RI_MMI
               && p_sessions[i_session_id - 1].i_slot == i_slot )
         {
            MMISendObject(i_session_id, &p_cmd->object );
            return libcLdvben50221::RET_OK;
         }
      }
